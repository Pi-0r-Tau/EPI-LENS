...
function frameHistogramDiff(data1, data2) {
    ...
    const rWeight = 0.2126, gWeight = 0.7152, bWeight = 0.0722; // sRGB luminance
    ...
    for (let i = 0; i < data1.length; i += 4) {
        ...
        const lum1 = srgb2linear(data1[i]) * rWeight +
                     srgb2linear(data1[i + 1]) * gWeight +
                     srgb2linear(data1[i + 2]) * bWeight;
        const lum2 = srgb2linear(data2[i]) * rWeight +
                     srgb2linear(data2[i + 1]) * gWeight +
                     srgb2linear(data2[i + 2]) * bWeight;

        const v1 = floor(lum1 * 255);
        const v2 = floor(lum2 * 255);
        hist1[min(bins - 1, floor(v1 / binSize))]++;
        hist2[min(bins - 1, floor(v2 / binSize))]++;
    }

    let diff = 0, total = 0;
    for (let i = 0; i < bins; ++i) {
        const binDiff = abs(hist1[i] - hist2[i]);
        // Ignore tiny changes

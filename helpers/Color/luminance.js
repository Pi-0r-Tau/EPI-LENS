// 230.1.O Regression testing indicated that default param allocation of the weights  was a bottleneck so have cached it here. 
// Having weights declared here as having issues in how they are accessed per call
const _DEFAULT_WEIGHTS = [0.2126, 0.7152, 0.0722];
...
(function() {
  ...
  window.AnalyzerHelpers.luminance = function(data, idx, weights) {
    const w = weights || _DEFAULT_WEIGHTS;
    return LUT[data[idx]] * w[0] +
           LUT[data[idx + 1]] * w[1] +
           LUT[data[idx + 2]] * w[2];
  };
  ...
  window.AnalyzerHelpers.luminance255 = function(data, idx, weights) {
    const w = weights || _DEFAULT_WEIGHTS;
    return LUT255[data[idx]] * w[0] +
           LUT255[data[idx + 1]] * w[1] +
           LUT255[data[idx + 2]] * w[2];
  };
})();